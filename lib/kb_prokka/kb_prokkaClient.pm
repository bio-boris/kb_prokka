package kb_prokka::kb_prokkaClient;
...
sub new
{
    ...
    my $self = {
	client => kb_prokka::kb_prokkaClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a kb_prokka.AnnotateParams
$return is a kb_prokka.AnnotateOutput
AnnotateParams is a reference to a hash where the following keys are defined:
	object_ref has a value which is a kb_prokka.data_obj_ref
	...
	metagenome has a value which is a kb_prokka.boolean
	rawproduct has a value which is a kb_prokka.boolean
	fast has a value which is a kb_prokka.boolean
	mincontiglen has a value which is an int
	evalue has a value which is a string
	rfam has a value which is a kb_prokka.boolean
	norrna has a value which is a kb_prokka.boolean
	notrna has a value which is a kb_prokka.boolean
data_obj_ref is a string
boolean is an int
AnnotateOutput is a reference to a hash where the following keys are defined:
	output_genome_ref has a value which is a kb_prokka.genome_ref
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for kb_prokka::kb_prokkaClient\n";
    }
    if ($sMajor == 0) {
        warn "kb_prokka::kb_prokkaClient version is $svr_version. API subject to change.\n";
    }
}
...
package kb_prokka::kb_prokkaClient::RpcClient;
